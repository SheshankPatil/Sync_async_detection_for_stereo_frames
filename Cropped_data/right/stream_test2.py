...
def process_image(image_path, output_prefix):
    with vpi.Backend.CUDA:
        ...
        input_vpi = vpi.asimage(input_image_resized)
...
def process_image_for_contours(image_path, contour_output_folder):
    ...
    # Save contour-mapped images for later reference
    red_contour_image = cv2.drawContours(np.zeros_like(input_image), contours_red, -1, (255, 0, 0), 2)
    white_contour_image = cv2.drawContours(np.zeros_like(input_image), contours_white, -1, (0, 255, 0), 2)

    red_contour_output_path = os.path.join(contour_output_folder, f"{os.path.splitext(os.path.basename(image_path))[0]}_red_contours.png")
    ...
    cv2.imwrite(red_contour_output_path, red_contour_image)
...
def process_images_in_folders(left_folder_path, right_folder_path, contour_output_folder):
    ...
    for left_file, right_file in zip(left_files, right_files):
        ...
        left_red, left_white = process_image_for_contours(left_output, contour_output_folder)
        right_red, right_white = process_image_for_contours(right_output, contour_output_folder)
...
left_folder_path = 'NEW_TEST_OWN/new_30fps/NEW_LEFT'
right_folder_path = 'NEW_TEST_OWN/new_30fps/NEW_RIGHT'
